 ...
 *
 * Obtain a handle from the [`deconstructionRecipeDesc`] property on [`RemoteTables`],
 * like `ctx.db.deconstructionRecipeDesc`.
 *
...
export class DeconstructionRecipeDescTableHandle {
   ...
   * Users are encouraged not to explicitly reference this type,
   * but to directly chain method calls,
   * like `ctx.db.deconstructionRecipeDesc.id().find(...)`.
   *
