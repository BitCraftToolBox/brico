 ...
 *
 * Obtain a handle from the [`playerUsernameState`] property on [`RemoteTables`],
 * like `ctx.db.playerUsernameState`.
 *
...
export class PlayerUsernameStateTableHandle {
   ...
   * Users are encouraged not to explicitly reference this type,
   * but to directly chain method calls,
   * like `ctx.db.playerUsernameState.entityId().find(...)`.
   *
   * Get a handle on the `entityId` unique index on the table `player_username_state`.
   */
  entityId = {
   ...
   * Users are encouraged not to explicitly reference this type,
   * but to directly chain method calls,
   * like `ctx.db.playerUsernameState.username().find(...)`.
   *
